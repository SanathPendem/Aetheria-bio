...
export const ImpactSection: React.FC = () => {
  ...
  // Animated counter effect on scroll/render
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setCounts({
        ...
        accuracy: parseFloat((99.8 * Math.min(1, progress)).toFixed(1)),
  ...
  const researchHubs = [
    ...
    { city: 'Zurich', role: 'LNP Nanocarrier & Transcytosis Lab', status: 'Active', LatLng: '47.3769° N, 8.5417° E' },
    { city: 'Tokyo', role: 'Organoid & Tissue-Chip Microfluidics', status: 'Active', LatLng: '35.6762° N, 139.6503° E' },
    { city: 'Singapore', role: 'Phase I/II Clinical Trial Management', status: 'Active', LatLng: '1.3521° N, 103.8198° E' },
  ];

  return (
    <section id="impact" className="relative py-24 bg-[#070A11] bio-grid-pattern overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {IMPACT_STATS.map((stat) => {
            ...
            return (
              <div
                key={stat.id}
                className="p-8 rounded-3xl glass-panel glass-panel-hover border border-white/10 text-center space-y-3 relative group overflow-hidden"
              >
                ...
                <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-cyan-500 to-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            );
          })}
        </div>
        ...
        <div className="p-8 rounded-3xl glass-panel border border-cyan-500/30 space-y-8 shadow-2xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-outfit text-2xl font-bold text-white">Global Research & Clinical Node Network</h3>
            </div>
            <span className="text-xs font-mono text-emerald-400 flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
              4 International Infrastructure Nodes Active
            </span>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {PUBLICATIONS_DATA.map((pub) => (
              <div key={pub.id} className="p-6 rounded-2xl glass-panel border border-white/10 space-y-4 flex flex-col justify-between hover:border-cyan-500/40 transition-colors">
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs font-mono text-cyan-400">
                    <span>{pub.journal} ({pub.year})</span>
                    <span className="px-2 py-0.5 rounded bg-cyan-950/80 border border-cyan-500/30 text-[10px]">IF: {pub.impactFactor}</span>
                  </div>
